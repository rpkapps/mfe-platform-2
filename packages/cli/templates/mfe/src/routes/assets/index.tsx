...
export const Route = createFileRoute("/assets/")({
  staticData: { breadcrumb: "Assets", navigation: { title: "Assets", description: "All tracked assets", keywords: ["list"], order: 1 } },
  // Loaders read the runtime env through the route context; they re-run when the context changes.
  loader: ({ context, abortController }) => fetchAssets(context.platform.runtime.env.API_BASE_URL, abortController.signal),
  component: AssetList,
})
    ...
    <ul className="flex flex-col divide-y divide-border text-sm">
      {assets.map((asset) => (
        <li key={asset.id} className="flex items-center gap-3 py-2">
          ...
          <span className="ml-auto text-muted-foreground">{asset.site}</span>
        </li>
      ))}
    </ul>
  )
