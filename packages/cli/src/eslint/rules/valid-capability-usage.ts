...
export default createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "SDK APIs that imply a capability are not used when mfe.config.ts removes it, and useCapability() names a known capability.",
    },
    messages: {
      removedCapability:
        "`{{api}}` requires the `{{capability}}` capability, which mfe.config.ts removes (`capabilities.remove`). Remove it from the list or stop using the API; the host would not grant it.",
  ...
  create(context) {
    ...
    const removed = new Set(
      Array.isArray(capabilities.remove)
        ? capabilities.remove.filter((entry): entry is string => typeof entry === "string")
        : []
    )
      ...
      CallExpression(node) {
        ...
        if (id !== null && !isCapabilityId(id)) {
          context.report({
            ...
            data: { id, known: CAPABILITY_IDS.join(", ") },
          })
        }
      },
    }
