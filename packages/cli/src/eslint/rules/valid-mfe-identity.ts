...
export default createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "One createMfe() per module, default-exported from src/mfe.tsx, with kebab-case widget keys.",
    },
    messages: {
      multipleCreateMfe:
        "`createMfe()` is called more than once in this file; an MFE has exactly one definition (src/mfe.tsx). Widgets belong in `createMfe({ widgets })`, not in separate definitions.",
      defaultExport:
        "`src/mfe.tsx` must default-export the `createMfe({...})` call (directly, or a binding holding it): the generated entry imports the default export as the remote definition.",
      widgetKey:
        'Widget key "{{key}}" must be kebab-case (`asset-card`): it becomes the widget id in the manifest and in `WidgetSlot` references.',
    },
    schema: [],
  },
  defaultOptions: [],
    ...
    const isCreateMfeCall = (node: TSESTree.Node, depth = 0): boolean => {
      ...
      if (value.type === AST_NODE_TYPES.CallExpression) {
        ...
        if (
          ...
          (callee.name === "createMfe" || imports.get(callee.name) === "createMfe")
        )
          return true
        if (
          callee.type === AST_NODE_TYPES.MemberExpression &&
          callee.property.type === AST_NODE_TYPES.Identifier &&
          callee.property.name === "createMfe"
        )
          return true
        if (
          depth < 3 &&
          value.arguments.some(
            (argument) =>
              ...
              isCreateMfeCall(argument, depth + 1)
          )
        )
          return true
        if (
          depth < 3 &&
          callee.type === AST_NODE_TYPES.MemberExpression &&
          isCreateMfeCall(callee.object, depth + 1)
        )
          return true
      }
    ...
    return {
      ...
      VariableDeclarator(node) {
        if (
          ...
          node.init &&
          isCreateMfeCall(node.init)
        )
          createMfeBindings.add(node.id.name)
      },
      CallExpression(node) {
        const name =
        ...
        if (widgetsObject) {
          for (const property of objectProperties(widgetsObject)) {
            ...
            if (key !== null && !KEBAB_LOCAL_ID_RE.test(key))
              context.report({ node: property.key, messageId: "widgetKey", data: { key } })
          }
        }
      },
      ...
      "Program:exit"() {
        ...
        if (!isCreateMfeCall(declaration))
          context.report({ node: defaultExport, messageId: "defaultExport" })
      },
    }
  },
})
