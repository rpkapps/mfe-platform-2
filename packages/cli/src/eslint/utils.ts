...
export function readIdentity(projectRoot: string): { mfeId: string } | null {
  const identity = readCachedJson<{ mfeId?: unknown }>(
    identityCache,
    join(projectRoot, ".platform", "identity.json")
  )
...
export function readPackageJson(projectRoot: string): Record<string, unknown> | null {
  return readCachedJson<Record<string, unknown>>(
    packageCache,
...
export function propertyKeyName(
  property: TSESTree.Property | TSESTree.JSXAttribute
): string | null {
  if (property.type === AST_NODE_TYPES.JSXAttribute)
    return property.name.type === AST_NODE_TYPES.JSXIdentifier ? property.name.name : null
...
export function findProperty(
  ...
  for (const property of object.properties) {
    if (property.type === AST_NODE_TYPES.Property && propertyKeyName(property) === name)
      return property
  }
  return null
}
...
export function stringValue(node: TSESTree.Node | null | undefined): string | null {
  ...
  if (node.type === AST_NODE_TYPES.JSXExpressionContainer)
    return stringValue(node.expression as TSESTree.Node)
  return null
}
...
export function unwrapExpression(node: TSESTree.Node): TSESTree.Node {
  ...
  for (;;) {
    if (
      ...
      current.type === AST_NODE_TYPES.JSXExpressionContainer &&
      current.expression.type !== AST_NODE_TYPES.JSXEmptyExpression
    ) {
      current = current.expression
    } else {
      return current
...
export function enclosingFunctionName(node: TSESTree.Node): {
  found: boolean
  name: string | null
...
export function jsxAttribute(
  ...
  for (const attribute of node.attributes) {
    if (attribute.type === AST_NODE_TYPES.JSXAttribute && propertyKeyName(attribute) === name)
      return attribute
  }
  return null
}
...
export const IMPERATIVE_REGISTERS = [
  "registerCommand",
 ...
 */
export function globalIdentifierReferences(
  ...
  for (const reference of scope.through) {
    if (names.has(reference.identifier.name))
      found.push(reference.identifier as TSESTree.Identifier)
  }
