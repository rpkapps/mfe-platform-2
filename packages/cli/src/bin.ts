...
cli
  ...
  .option("--tecton", "Use the Tecton UI library (default) — --no-tecton for a plain project", { default: true })
  .option("--package-manager <pm>", "pnpm | npm | yarn", { default: "pnpm" })
  .option("--install", "Install dependencies after scaffolding (default) — --no-install to skip", { default: true })
  .option("--git", "Initialise a git repository (default) — --no-git to skip", { default: true })
  .option("--link-platform <monorepoRoot>", "Link @platform/* and @tecton/react from a platform monorepo checkout instead of a registry")
  ...
  .action((name: string, options: GlobalOptions & { template: string; react: string; tecton: boolean; packageManager: "pnpm" | "npm" | "yarn"; install: boolean; git: boolean; linkPlatform?: string; force?: boolean; dir?: string; mfeId?: string; displayName?: string }) =>
    run(async () => {
      ...
      console.log(pc.dim("Read AGENTS.md / llm.txt in the project for the canonical patterns."))
    })
  )
...
cli
  ...
  .option("--harness", "Serve the local shell harness (default) — --no-harness to disable", { default: true })
  .action((options: GlobalOptions & { port?: string; host?: string | boolean; open?: boolean; harness: boolean }) =>
    run(async () => {
      await dev({ cwd: cwdOf(options), port: options.port ? Number.parseInt(String(options.port), 10) : undefined, host: options.host, open: options.open, harness: options.harness })
    })
  )
...
cli
  ...
  .action((options: GlobalOptions & { out?: string; json?: boolean; mode: "build" | "dev" }) =>
    run(async () => {
      await manifest({ cwd: cwdOf(options), out: options.out, json: options.json, mode: options.mode })
    })
  )

cli
  .command("validate", "Validate the project structure, identity, configuration, generated files and manifest")
  .option("--no-manifest", "Skip manifest generation")
  .action((options: GlobalOptions & { manifest: boolean }) =>
    run(async () => {
      const result = await validate({ cwd: cwdOf(options), manifest: options.manifest, log: (message) => console.log(message) })
      if (!result.ok) process.exit(1)
    })
  )

cli
  .command("lint [...files]", "Run ESLint with the project's eslint.config (the platform shareable config)")
...
cli
  ...
  .action((filters: string[], options: GlobalOptions & { watch?: boolean; coverage?: boolean; e2e?: boolean }) =>
    run(async () => {
      const result = await test({ cwd: cwdOf(options), watch: options.watch, coverage: options.coverage, e2e: options.e2e, filters })
      if (!result.ok) process.exit(1)
    })
  )
