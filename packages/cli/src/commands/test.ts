...
interface VitestNodeModule {
  startVitest(cliFilters?: string[], options?: Record<string, unknown>, viteOverrides?: Record<string, unknown>): Promise<{
    close(): Promise<void>
...
export async function test(options: TestOptions = {}): Promise<TestResult> {
  ...
  if (options.e2e) {
    const cli = resolveProjectModule(root, "@playwright/test/cli") ?? resolveProjectModule(root, "playwright/cli")
    const args = ["test", ...(options.filters ?? [])]
    const code = cli ? await runNode(cli, args, root) : await runBinary("npx", ["playwright", ...args], root)
    if (!cli && code !== 0) {
      throw new CliError({ code: "TESTS_FAILED", message: "Playwright is not installed in the project (and `npx playwright test` failed).", source: root, override: "pnpm add -D @playwright/test && pnpm exec playwright install chromium" })
    }
    return { ok: code === 0, failed: code === 0 ? 0 : 1, mode: "e2e" }
  }
  const { startVitest } = await loadProjectModule<VitestNodeModule>(root, "vitest/node", "run unit tests")
  const vitest = await startVitest(options.filters ?? [], { root, watch: Boolean(options.watch), ...(options.coverage ? { coverage: { enabled: true } } : {}) })
