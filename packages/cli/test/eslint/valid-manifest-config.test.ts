...
ruleTester.run("valid-manifest-config", rule, {
  ...
  invalid: [
    ...
    { code: `${imports}export default defineMfeConfig({ shared: { lodash: { scope: "shared", eager: true } } })`, filename: file, errors: [{ messageId: "sharedScope", data: { key: "lodash", value: "shared" } }, { messageId: "unknownKey", data: { key: "shared.lodash.eager", valid: "version, bundle, singleton, scope" } }] },
    { code: `${imports}export default defineMfeConfig({ capabilities: { add: ["teleportation"], deny: [] } })`, filename: file, errors: [{ messageId: "unknownCapability", data: { id: "teleportation", list: "add", known: "navigation, context, storage.local, storage.session, telemetry, commands, settings, help, release-notes, breadcrumbs, overlays, notifications, runtime-env, widgets" } }, { messageId: "unknownKey", data: { key: "capabilities.deny", valid: "add, remove" } }] },
    { code: `${imports}export default defineMfeConfig({ navigation: { title: 1, colour: "red" } })`, filename: file, errors: [{ messageId: "navigationTitle" }, { messageId: "unknownKey", data: { key: "navigation.colour", valid: "title, description, icon, keywords, category, order" } }] },
