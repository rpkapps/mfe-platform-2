...
function problems(results: LintResult[], cwd: string): string[] {
  return results.flatMap((result) => result.messages.map((message) => `${result.filePath.slice(cwd.length + 1)}:${message.line} ${message.ruleId ?? "parse"} ${message.message}`))
}
...
describe("platform create → platform lint", () => {
  it("scaffolds a React 19 Tecton project that passes the platform lint configuration without manual setup", async () => {
    const result = await create({ name: "@acme/asset-tracker", dir: temp.dir, linkPlatform: MONOREPO_ROOT, install: false, git: false })
    const results = await lintProject(result.dir)
    const lintedFiles = results.map((entry) => entry.filePath.slice(result.dir.length + 1).replace(/\\/g, "/"))
    expect(lintedFiles).toEqual(expect.arrayContaining(["src/mfe.tsx", "src/routes/index.tsx", "src/routes/settings.tsx", "mfe.config.ts", "eslint.config.ts", "src/widgets/asset-card.tsx"]))
  ...
  it("scaffolds a React 18 plain project that passes as well", async () => {
    const result = await create({ name: "legacy-reports", dir: temp.dir, react: 18, tecton: false, linkPlatform: MONOREPO_ROOT, install: false, git: false })
    const results = await lintProject(result.dir)
