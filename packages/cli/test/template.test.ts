...
const context = {
  flags: { tecton: true, react18: false },
  tokens: { MFE_ID: "asset-tracker", DISPLAY_NAME: "Asset Tracker" },
}

describe("renderTemplate", () => {
  it("replaces tokens and leaves unknown tokens alone", () => {
    expect(replaceTokens("__MFE_ID__/__DISPLAY_NAME__/__UNKNOWN__", context.tokens)).toBe(
      "asset-tracker/Asset Tracker/__UNKNOWN__"
    )
    expect(renderTemplate('const id = "__MFE_ID__"', context)).toBe(
      'const id = "asset-tracker"'
    )
  })
  ...
  it("accepts markers wrapped in comments so template files stay valid source", () => {
    ...
    expect(renderTemplate(source, context)).toBe(
      ['import x from "x"', "doc", "shell"].join("\n")
    )
  })
  ...
  it("supports nested and inline blocks", () => {
    ...
    expect(
      renderTemplate("React {{#react18}}18{{/react18}}{{^react18}}19{{/react18}}", context)
    ).toBe("React 19")
  })
