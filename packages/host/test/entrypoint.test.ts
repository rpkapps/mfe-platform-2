...
describe("entrypoint", () => {
  it("generates a validated document from PLATFORM_* variables, refusing sensitive names", () => {
    const dir = tmp()
    writeFileSync(join(dir, "base.json"), JSON.stringify({ environment: "base", mfes: { "asset-tracker": { env: { FROM_BASE: 1 } } } }))
    const result = generateRuntimeConfigFile({
      cwd: dir,
      out: "out/platform-config.json",
      base: "base.json",
      known: ["asset-tracker"],
      env: { PLATFORM_ENVIRONMENT: "staging", PLATFORM_MFE_ASSET_TRACKER_MANIFEST_URL: "https://cdn/m.json", PLATFORM_MFE_ASSET_TRACKER_ENV_API_BASE_URL: "https://api", PLATFORM_MFE_ASSET_TRACKER_ENABLED: "false", PLATFORM_SHARED_REGION: "eu", PLATFORM_API_SECRET: "nope", PLATFORM_MFE_ASSET_TRACKER_ENV_TOKEN: "nope", HOME: "/root" },
      now: () => new Date("2026-01-01T00:00:00Z"),
    })
    expect(result.refused.sort()).toEqual(["PLATFORM_API_SECRET", "PLATFORM_MFE_ASSET_TRACKER_ENV_TOKEN"])
    expect(result.sources).toMatchObject({ environment: "PLATFORM_ENVIRONMENT", "mfes.asset-tracker.manifestUrl": "PLATFORM_MFE_ASSET_TRACKER_MANIFEST_URL", "shared.REGION": "PLATFORM_SHARED_REGION" })
    expect(result.config).toMatchObject({ environment: "staging", source: "entrypoint", shared: { REGION: "eu" }, mfes: { "asset-tracker": { enabled: false, manifestUrl: "https://cdn/m.json", env: { FROM_BASE: 1, API_BASE_URL: "https://api" } } } })
  ...
  it("parses CLI arguments and exits non-zero on invalid input", () => {
    expect(parseArgs(["--out", "x.json", "--base=b.json", "--known", "a,b", "--print"])).toEqual({ out: "x.json", base: "b.json", known: ["a", "b"], print: true, help: false })
    ...
    const dir = tmp()
    expect(runEntrypoint(["--out", "config.json"], { PLATFORM_ENVIRONMENT: "prod", PLATFORM_DB_PASSWORD: "x" }, io, dir)).toBe(0)
