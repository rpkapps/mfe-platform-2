...
describe("search index", () => {
  it("ranks commands, navigation, settings, help, release notes and breadcrumbs", async () => {
    ...
    host.registries.settings.register(
      {
        ...
        title: "Display",
        fields: {
          density: { defaultValue: "comfortable", label: "Density", keywords: ["compact"] },
        },
      },
      owner
    )
    ...
    host.registries.releaseNotes.register(
      { id: "v2", version: "2.0.0", title: "Big release", summary: "Export improvements" },
      owner
    )
    ...
    expect(exportResults.find((entry) => entry.label === "Restricted export")?.disabled).toBe(
      true
    )
    expect(exportResults.some((entry) => entry.kind === "release-note")).toBe(true)
    expect(index.search("csv")[0]?.label).toBe("Export assets")
    expect(index.search("compact")[0]).toMatchObject({
      kind: "setting",
      label: "Density",
      settings: { groupKey: "display", fieldKey: "density" },
    })
    expect(index.search("assets", { kinds: ["navigate"] })[0]).toMatchObject({
      kind: "navigate",
      href: "/asset-tracker",
    })
    expect(index.search("asset details")[0]).toMatchObject({
      kind: "navigate",
      href: "/asset-tracker/assets/$assetId",
    })
    expect(index.search("advanced")[0]).toMatchObject({
      kind: "setting",
      href: "/asset-tracker/settings",
    })
    expect(index.search("getting")[0]).toMatchObject({
      kind: "help",
      qualifiedId: "asset-tracker:intro",
    })
    expect(index.search("home")[0]).toMatchObject({ kind: "breadcrumb", href: "/" })
    expect(index.search("go assets")[0]).toMatchObject({
      kind: "command",
      qualifiedId: "asset-tracker:go-assets",
...
describe("runCommand", () => {
  it("reports states, telemetry and diagnostics; never throws", async () => {
    ...
    host.registries.commands.register(
      { id: "nav", label: "Nav", route: "/assets/1", handler: undefined as never },
      owner
    )
    expect(await runCommand(host, "asset-tracker:ok", { source: "palette" })).toMatchObject({
      outcome: "succeeded",
    })
    expect(await runCommand(host, "asset-tracker:boom")).toMatchObject({
      outcome: "failed",
      error: expect.objectContaining({ code: "COMMAND_FAILED" }),
    })
    expect(host.registries.commands.states()["asset-tracker:boom"]).toMatchObject({
      status: "failed",
      error: "nope",
    })
    expect(await runCommand(host, "asset-tracker:missing")).toMatchObject({
      outcome: "unknown",
    })
    ...
    expect(
      host.diagnostics
        .list({ type: "command.run" })
        .map((event) => (event as { outcome: string }).outcome)
    ).toEqual(["started", "succeeded", "started", "failed", "failed", "started", "succeeded"])
    expect(
      host
        .telemetryEvents()
        .some((event) => event.kind === "span" && event.name === "command.run")
    ).toBe(true)
  })
  it("aborts long-running commands and hides unavailable ones", async () => {
    ...
    host.registries.commands.register(
      { id: "hidden", label: "Hidden", availability: () => false, handler: () => {} },
      owner
    )
    ...
    expect(await host.commands.run("asset-tracker:restricted")).toMatchObject({
      outcome: "unavailable",
    })
    expect(await host.commands.run("asset-tracker:hidden")).toMatchObject({
      outcome: "unavailable",
    })
...
describe("installShortcutListener", () => {
  it("dispatches shortcuts once per target and ignores editable targets", async () => {
    ...
    const handler = vi.fn()
    host.registries.commands.register(
      { id: "palette", label: "Palette", shortcut: "mod+shift+p", handler },
      owner
    )
