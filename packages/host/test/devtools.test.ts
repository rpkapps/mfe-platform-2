...
describe("shouldLoadDevtools", () => {
  it("evaluates the flag/policy/environment matrix", () => {
    ...
    expect(decideDevtools(hostWith("flag", "test"))).toEqual({
      allowed: false,
      reason: "flag-missing",
    })
    expect(decideDevtools(hostWith("never", "test"))).toEqual({
      allowed: false,
      reason: "policy-never",
    })
    expect(decideDevtools(hostWith("always", "production"))).toEqual({
      allowed: true,
      reason: "policy-always",
    })
    expect(decideDevtools(hostWith("flag", "production"))).toEqual({
      allowed: false,
      reason: "environment",
    })
    ...
    expect(decideDevtools(hostWith("flag", "production", ["production"]))).toEqual({
      allowed: true,
      reason: "flag",
    })
    expect(decideDevtools(hostWith("never", "test"))).toEqual({
    ...
    expect(
      decideDevtools(hostWith("flag", "test"), {
        location: { search: "?platform.devtools=1" },
        localStorage: window.localStorage,
      } as unknown as Window)
    ).toEqual({ allowed: true, reason: "flag" })
    expect(decideDevtools(hostWith("always", "test"), null)).toEqual({
      allowed: false,
      reason: "no-window",
    })
    const host = hostWith("never", "test")
    expect(shouldLoadDevtools(host)).toBe(false)
    expect(host.diagnostics.list({ type: "devtools" })[0]).toMatchObject({
      action: "denied",
      reason: "policy-never",
    })
  })
