...
describe("manifest URL precedence", () => {
  it("query > local override > runtime config > registry > default", () => {
    ...
    const withQuery = createOverrideStore({
      storage: sessionStorage,
      search: "?platform.override.asset-tracker=http%3A%2F%2Flocalhost%3A4444%2Fm.json&other=1",
    })
    expect(resolveManifestUrl({ ...base, overrides: withQuery })).toEqual({
      url: "http://localhost:4444/m.json",
      source: "query",
    })
    ...
    expect(JSON.parse(sessionStorage.get("session", MANIFEST_OVERRIDES_KEY)!)).toEqual({
      "asset-tracker": "http://localhost:4444/m.json",
    })
    expect(
      resolveManifestUrl({
        ...base,
        overrides: createOverrideStore({ storage: sessionStorage, search: "" }),
      }).source
    ).toBe("query")
    noQuery.setLocal("asset-tracker", null)
    expect(storage.get("local", MANIFEST_OVERRIDES_KEY)).toBeNull()
    expect(
      resolveManifestUrl({ ...base, overrides: noQuery, runtimeConfig: parseRuntimeConfig({}) })
    ).toEqual({ url: "/registry/m.json", source: "registry" })
    expect(
      resolveManifestUrl({
        ...base,
        overrides: noQuery,
    ...
    ).toEqual({ url: "/mfes/asset-tracker/platform-manifest.json", source: "default" })
  })
})
...
describe("origin validation", () => {
  ...
  it("allows same-origin, runtime config, per-MFE, registry and policy origins", () => {
    ...
    expect(
      assertOriginAllowed({
        ...base,
        url: "https://cdn.example.com/m.json",
        runtimeConfig: parseRuntimeConfig({ allowedOrigins: ["https://cdn.example.com"] }),
      })
    ).toBe("https://cdn.example.com")
    expect(
      assertOriginAllowed({
        ...base,
        url: "https://cdn2.example.com/m.json",
        runtimeConfig: parseRuntimeConfig({
          mfes: { "asset-tracker": { allowedOrigins: ["https://cdn2.example.com"] } },
        }),
      })
    ).toBe("https://cdn2.example.com")
    expect(
      assertOriginAllowed({
        ...base,
        url: "https://reg.example.com/m.json",
        runtimeConfig: parseRuntimeConfig({}),
        registry: { mfeId: "asset-tracker", manifestUrl: "https://reg.example.com/x.json" },
      })
    ).toBe("https://reg.example.com")
    expect(
      assertOriginAllowed({
        ...base,
        url: "http://localhost:5173/m.json",
        runtimeConfig: parseRuntimeConfig({}),
        policyOrigins: ["http://localhost:5173"],
      })
  ...
  it("denies everything else with MANIFEST_ORIGIN_DENIED", () => {
    expect(() =>
      assertOriginAllowed({
        ...base,
        url: "https://evil.example.com/m.json",
        runtimeConfig: parseRuntimeConfig({ allowedOrigins: ["https://cdn.example.com"] }),
      })
    ).toThrowError(expect.objectContaining({ code: "MANIFEST_ORIGIN_DENIED" }))
    expect(() =>
      assertOriginAllowed({ ...base, url: "http://", runtimeConfig: parseRuntimeConfig({}) })
    ).toThrowError(expect.objectContaining({ code: "MANIFEST_ORIGIN_DENIED" }))
  })
})
...
describe("fetchManifest", () => {
  ...
  it("retries with backoff and cache-busts retries", async () => {
    ...
    expect(fetch.calls.map((call) => call.init?.cache)).toEqual([
      "default",
      "no-store",
      "no-store",
    ])
  ...
  it("does not retry invalid manifests (MANIFEST_INVALID) or mismatched ids", async () => {
    ...
    ).rejects.toMatchObject({ code: "MANIFEST_FETCH_FAILED", details: { status: 404 } })
  })
})
