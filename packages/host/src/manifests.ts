...
export function createOverrideStore(options: {
  storage: StorageBackend
  search?: string
}): OverrideStore {
  ...
  if (search) {
    try {
      ...
      for (const [key, value] of params.entries()) {
        if (key.startsWith(MANIFEST_QUERY_PREFIX) && value)
          fromQuery[key.slice(MANIFEST_QUERY_PREFIX.length)] = value
      }
    } catch {
  ...
  if (Object.keys(fromQuery).length)
    writeJsonRecord(storage, "session", { ...readJsonRecord(storage, "session"), ...fromQuery })
  return {
...
export function resolveManifestUrl(input: ResolveManifestUrlInput): ManifestUrlResolution {
  ...
  if (input.registry?.manifestUrl)
    return { url: input.registry.manifestUrl, source: "registry" }
...
export function assertOriginAllowed(input: OriginPolicyInput): string {
  ...
  for (const value of input.runtimeConfig.mfes[input.mfeId]?.allowedOrigins ?? [])
  ...
  throw new PlatformError({
    ...
    source: input.url,
    override:
      "PLATFORM_ALLOWED_ORIGINS or `mfes.<mfeId>.allowedOrigins` in the runtime configuration",
...
): Promise<{ manifest: MfeManifest; url: string; attempts: number }> {
  ...
  for (let attempt = 1; attempt <= options.attempts + 1; attempt += 1) {
    ...
    try {
      const response = await options.fetch(url, {
        cache: options.noStore || attempt > 1 ? "no-store" : "default",
      ...
      if (!response.ok)
        throw new PlatformError({
          code: "MANIFEST_FETCH_FAILED",
          message: `HTTP ${response.status} while fetching the manifest of ${options.mfeId}.`,
          owner: { mfeId: options.mfeId },
          source: url,
      ...
      try {
        ...
        throw new PlatformError({
          ...
          message: `The manifest of ${options.mfeId} is not valid JSON.`,
          owner: { mfeId: options.mfeId },
          source: url,
          cause: error,
          details: { fatal: true },
      ...
      if (result.manifest.mfeId !== options.mfeId) {
        throw new PlatformError({
          code: "MANIFEST_INVALID",
          message: `The manifest at ${url} belongs to "${result.manifest.mfeId}", not "${options.mfeId}".`,
          owner: { mfeId: options.mfeId },
          source: url,
          details: { fatal: true },
        })
      ...
      const platformError =
        error instanceof PlatformError
          ...
          : new PlatformError({
              code: "MANIFEST_FETCH_FAILED",
              message: `Fetching the manifest of ${options.mfeId} failed: ${error instanceof Error ? error.message : String(error)}`,
              owner: { mfeId: options.mfeId },
              source: url,
              cause: error,
              details: { attempt },
      ...
      if (
        platformError.details?.fatal ||
        platformError.details?.aborted ||
