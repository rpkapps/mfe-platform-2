...
export interface FailureLab {
  state: {
    manifest404: Set<string>
    droppedCapabilities: Set<CapabilityId>
    denyGroups: boolean
...
export function readHarnessManifestUrls(win: Window = window): string[] {
  ...
  try {
    fromQuery = new URLSearchParams(win.location.search)
      .getAll("manifest")
...
export function createBasedNavigation(
  inner: ShellNavigation & { dispose?(): void },
  basePath: string
  ...
  const add = (href: string) =>
    href.startsWith("/") ? `${basePath}${href === "/" ? "" : href}` || "/" : href
  const map = (location: ReturnType<ShellNavigation["getLocation"]>) => ({
    ...location,
    pathname: strip(location.pathname),
  })
  return {
    ...
    reload: () => inner.reload(),
    subscribe: (listener) =>
      inner.subscribe((location, action) => listener(map(location), action)),
    ...
    async load(manifest, options) {
      if (lab.state.unavailable.has(manifest.mfeId)) {
        throw new PlatformError({
          ...
          message: `Failure lab: ${manifest.mfeId} is simulated as unavailable.`,
...
export async function connectManifest(
  ...
  try {
    ...
    host.diagnostics.emit({
      type: "log",
      level: "error",
      message: `Could not connect manifest ${url}: ${error instanceof Error ? error.message : String(error)}`,
    })
    return null
  }
}
...
export function mountHarness(options: HarnessOptions = {}): MountedHarness {
  ...
  let runtimeConfig: RuntimeConfig = parseRuntimeConfig(
    {
      environment: "local",
      devtools: { policy: "always", environments: ["local"] },
      allowedOrigins: origins,
      cache: { manifestMaxAgeSeconds: 0, bustOnRetry: true },
      retry: { attempts: 1, backoffMs: 300 },
      ...(options.runtimeConfig ?? window.__PLATFORM_HARNESS__?.runtimeConfig ?? {}),
    },
    "harness"
  )
  ...
  const baseLoader = createModuleFederationLoader({
    name: "harness",
    shared: harnessSharedModules(),
    retry: { attempts: 1, backoffMs: 300 },
  })
  const host = createPlatformHost({
    ...
    navigation,
    context: {
      ...DEFAULT_HARNESS_CONTEXT,
      ...(window.__PLATFORM_HARNESS__?.context ?? {}),
      ...(options.context ?? {}),
    },
    ...
    policy: {
      ...
      capabilities: (manifest) =>
        manifest.capabilities.filter(
          (capability) => !lab.state.droppedCapabilities.has(capability)
        ),
    },
    devtools: {
      policy: options.devtools === false ? "never" : "always",
      environments: [runtimeConfig.environment],
    },
