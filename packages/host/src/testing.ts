/**
 * `@platform/host/testing` — an in-memory host, a fake loader and a manifest
 * builder, so a shell can unit-test its own chrome without a browser, a
 * network or a real remote. Framework-free, like the rest of this package.
 */
import {
...
import type { PlatformHostOptions, RegistryEntry } from "./types"
...
/**
 * A `fetch` that answers from a URL map. `failures` makes a URL throw that many
 * times before succeeding (retry tests); `status` overrides the response status.
 */
