...
export function detectCommit(root: string): string | undefined {
  const fromEnv =
    process.env.GITHUB_SHA ?? process.env.CI_COMMIT_SHA ?? process.env.PLATFORM_COMMIT_SHA
...
export function buildManifestInput(
  config: ResolvedPlatformConfig,
  options: Omit<GenerateManifestOptions, "root" | "options" | "config"> = {}
): {
  input: MfeManifestInput
  routes: DerivedRoutes
  analysis: ProjectAnalysis
  ...
  const kind: MfeManifestInput["kind"] =
    !routes.hasRoutes && analysis.widgets.length > 0 ? "widget-library" : "mfe"
  const version = config.packageJson.version ?? "0.0.0"
  const builtAt = options.builtAt ?? new Date().toISOString()
  const buildId =
    options.buildId ??
    process.env.PLATFORM_BUILD_ID ??
    hashString(`${config.mfeId}@${version}@${builtAt}`)
  const permissionGroups = [
    ...new Set([
      ...config.permissionGroups,
      ...routes.routes.flatMap((route) => route.permissionGroups ?? []),
    ]),
  ]
  const capabilities = finalizeCapabilities(
  ...
  const input: MfeManifestInput = {
    ...
    entry: {
      ...
      file: REMOTE_ENTRY_FILE,
      expose: EXPOSE_KEY,
      type: "module",
      loaderManifest: MF_MANIFEST_FILE,
    },
    remote: {
      baseUrl:
        mode === "dev" && options.dev?.origin
          ? `${options.dev.origin.replace(/\/$/, "")}/`
          : "./",
      preload: "none",
    },
    ...
    tecton: {
      enabled: config.tecton,
      version: config.tectonVersion,
      cssProtocol: config.tecton ? TECTON_CSS_PROTOCOL : undefined,
      foundation: config.css.foundation,
    },
    css: {
      ownerAttribute: config.css.ownerAttribute,
      scoped: config.css.scope,
      assets: options.cssAssets ?? [],
    },
    env: { keys: envKeys },
  }
...
export async function generateManifest(
  ...
  const config =
    options.config ??
    (await resolvePlatformConfig({
      root: options.root,
      options: options.options,
      command: options.mode === "dev" ? "serve" : "build",
...
export function writeManifestCopy(
  config: ResolvedPlatformConfig,
  manifest: MfeManifest
): string {
