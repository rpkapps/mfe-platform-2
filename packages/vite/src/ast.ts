...
).default ?? traverseModule) as typeof traverseModule
  ...
  if (t.isObjectExpression(node)) {
    ...
    for (const property of node.properties) {
      if (!t.isObjectProperty(property))
        return { ok: false, reason: t.isSpreadElement(property) ? "object spread" : "method" }
...
): Map<string, t.ObjectProperty | t.ObjectMethod> {
...
export function propertyValue(
  property: t.ObjectProperty | t.ObjectMethod | undefined
): t.Node | undefined {
