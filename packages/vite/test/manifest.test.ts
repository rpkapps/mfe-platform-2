...
describe("generateManifest", () => {
  it("builds a valid production manifest from the fixture project", async () => {
    const generated = await generateManifest({
      root,
      mode: "build",
      cssAssets: ["assets/style-abc.css"],
    ...
    expect(manifest).toMatchObject({
      ...
      settings: [
        {
          ...
          managedBy: "framework",
          fields: [
            { key: "compact", label: "Compact mode", kind: "boolean" },
      ...
      runtime: {
        react: { requiredVersion: "^19.0.0", major: 19 },
        reactDom: { requiredVersion: "^19.0.0", major: 19 },
      },
      tecton: { enabled: false, foundation: "shell" },
      css: { ownerAttribute: "data-mfe", scoped: true, assets: ["assets/style-abc.css"] },
      env: {
        keys: {
          API_BASE_URL: { required: true, description: "Backend base URL", public: true },
          FEATURE_X: { required: false, default: false, public: true },
        },
      },
    })
    expect(manifest.routes.map((route) => route.fullPath)).toEqual([
      "/sample",
      "/sample/assets/$assetId",
      "/sample/settings",
    ])
    expect(manifest.routes.find((route) => route.path === "/settings")).toMatchObject({
      guarded: true,
      permissionGroups: ["sample:admin"],
    })
    expect(manifest.shared.map((request) => request.name)).toEqual([
      "@tanstack/react-router",
      "react",
      "react-dom",
    ])
    expect(
      manifest.shared.every((request) => request.shared && request.scope === "react19")
    ).toBe(true)
    expect(manifest.dev).toBeUndefined()
    expect(manifest.tecton.cssProtocol).toBeUndefined()
    expect(generated.warnings).toEqual([])
    expect(
      typeof manifest.release.commit === "string" || manifest.release.commit === undefined
    ).toBe(true)
  })
  ...
  it("adds the dev block in dev mode and derives the remote base URL from the origin", async () => {
    const generated = await generateManifest({
      ...
      dev: {
        ...
        restartReason: "package.json changed",
      },
    })
    expect(generated.manifest.dev).toEqual({
      hmr: true,
      origin: "http://localhost:4201",
  ...
  it("honours plugin options for the manifest and reports widget libraries", async () => {
    const generated = await generateManifest({
      ...
      options: {
        discoverable: false,
        displayName: "Renamed",
        capabilities: { remove: ["notifications", "settings"] },
        tecton: true,
        css: { scope: false, foundation: "bundled" },
        routesDirectory: "src/nowhere",
      },
    })
    expect(generated.manifest).toMatchObject({
      ...
      discoverable: false,
      routes: [],
      tecton: { enabled: true, cssProtocol: "1", foundation: "bundled" },
      css: { scoped: false },
    })
  ...
  it("fails with MANIFEST_INVALID when inferred data does not validate", async () => {
    await expect(
      generateManifest({
        ...
        persistIdentity: false,
        options: {
          navigation: { title: "" } as never,
          permissionGroups: [1 as unknown as string],
        },
      })
...
describe("generated entry and config hash", () => {
  it("renders the generated entry with and without Tecton", async () => {
    ...
    expect(renderEntry(plain)).toBe(
      '// Generated by @platform/vite — do not edit. Inspect freely.\nimport definition from "../src/mfe"\n\nexport default definition\n'
    )
    const tecton = await resolvePlatformConfig({
      root,
      persistIdentity: false,
      options: { tecton: true },
    })
    expect(renderEntry(tecton)).toBe(
  ...
  it("is stable for the same inputs and changes with manifest-affecting options", async () => {
    ...
    const c = await resolvePlatformConfig({
      root,
      persistIdentity: false,
      options: { shared: { react: { singleton: true } } },
    })
    expect(computeConfigHash(a, buildFederationConfig(a))).toBe(
      computeConfigHash(b, buildFederationConfig(b))
    )
    expect(computeConfigHash(a, buildFederationConfig(a))).not.toBe(
      computeConfigHash(c, buildFederationConfig(c))
    )
