...
import { analyzeProjectSources, analyzeSourceFile, finalizeCapabilities, listSourceFiles, mergeAnalyses } from "../src/analysis"

const dir = join(__dirname, "fixtures", "analysis")
const analyze = (name: string) => analyzeSourceFile(readFileSync(join(dir, name), "utf8"), join(dir, name))
...
describe("analyzeSourceFile", () => {
  it("collects commands from hooks, components and same-file constants", () => {
    ...
    expect(result.commands).toEqual([
      ...
      { id: "export-csv", label: "Export as CSV", description: "Download the current table", group: "Assets", keywords: ["download", "csv"], shortcut: "mod+shift+e", permissionGroups: ["assets:export"], route: "/assets", static: false },
  ...
  it("collects help entries and release notes from arrays, objects and components", () => {
    ...
    expect(result.capabilities.sort()).toEqual(["breadcrumbs", "help", "notifications", "release-notes", "runtime-env"])
    ...
    expect(result.releaseNotes).toEqual([{ id: "v1-2", version: "1.2.0", title: "Bulk export", date: "2026-01-01", summary: "Export many assets at once." }])
  })
  ...
  it("reads widgets and static registrations from createMfe", () => {
    ...
    expect(result.commands).toEqual([{ id: "open-assets", label: "Open assets", route: "/assets", static: true }])
    expect(result.help).toEqual([{ id: "overview", title: "Overview" }])
    expect(result.releaseNotes).toEqual([{ id: "v1-0", version: "1.0.0", title: "Initial release" }])
  })
  ...
  it("supports namespace imports and ignores files without SDK imports", () => {
    ...
    expect(namespaced.commands).toEqual([{ id: "ns-command", label: "Namespaced", static: false }])
  ...
  it("never throws on unparsable code", () => {
    const result = analyzeSourceFile('import { useRegisterCommand } from "@platform/react"\nexport const x = {{{', "broken.tsx")
    expect(result.capabilities).toEqual(["commands"])
    expect(result.warnings.some((warning) => warning.includes("could not parse")) || result.commands.length === 0).toBe(true)
  })
})
...
describe("project analysis", () => {
  it("lists source files without generated, declaration and test files", () => {
    ...
    expect(files).toEqual(["commands.tsx", "help.tsx", "mfe.tsx", "namespace.tsx", "plain.tsx", "routes/dynamic.tsx", "routes/guarded.tsx", "routes/lazy.lazy.tsx", "routes/plain.tsx", "settings.tsx"])
  })
  ...
  it("merges files, dedupes ids and attaches pending settings fields", () => {
    ...
    expect(project.files).toEqual(["commands.tsx", "help.tsx", "mfe.tsx", "namespace.tsx", "settings.tsx"])
    expect(project.widgets.map((widget) => widget.id)).toEqual(["status", "asset-count"])
    expect(project.warnings.some((warning) => warning.includes('duplicate widgets "status"'))).toBe(true)
    const display = project.settings.find((group) => group.key === "display")
    expect(display?.fields.map((field) => field.key)).toContain("extra")
    expect(project.settings.find((group) => group.key === "other")).toEqual({ key: "other", title: "other", managedBy: "framework", fields: [{ key: "flag", kind: "boolean" }] })
    expect(project.commands.map((command) => command.id)).toEqual(["create-asset", "export-csv", "refresh", "open-assets", "ns-command"])
    expect(project.capabilities.sort()).toEqual(["breadcrumbs", "commands", "context", "help", "navigation", "notifications", "release-notes", "runtime-env", "settings", "storage.local", "storage.session", "telemetry", "widgets"])
  })

  it("finalizes capabilities with implicit ones, additions and removals", () => {
    expect(finalizeCapabilities(["commands"], IMPLICIT_CAPABILITIES, ["widgets"], ["telemetry"])).toEqual(["commands", "context", "navigation", "overlays", "widgets"])
