...
export type DiagnosticEvent = DiagnosticBase &
  (
    ...
    | { type: "manifest.resolved"; url: string; urlSource: "override" | "runtime-config" | "registry" | "default" | "query"; cacheBusted: boolean }
    | { type: "manifest.loaded"; url: string; version: string; protocolVersion: string; dev: boolean }
    ...
    | { type: "shared.resolved"; resolutions: { name: string; scope: string; requiredVersion: string; outcome: "shared" | "bundled"; version?: string; from?: string; reason: string }[] }
    | { type: "mount.started"; container: string }
    | { type: "mount.completed"; durationMs: number; reactVersion?: string; routerVersion?: string }
    ...
    | { type: "route.guard"; routeId: string; outcome: "allowed" | "redirected" | "rejected"; detail?: string }
    | { type: "route.error"; routeId: string; error: string }
    | { type: "registration"; kind: "command" | "settings" | "help" | "release-notes" | "breadcrumbs"; action: "added" | "removed"; key: string }
    | { type: "shortcut.conflict"; shortcut: string; holder: string; rejected: string }
    | { type: "command.run"; qualifiedId: string; outcome: "started" | "succeeded" | "failed" | "cancelled"; durationMs?: number; error?: string }
    | { type: "settings.invalid"; key: string; message: string; recovered: "default" | "migrated" | "none" }
    | { type: "storage.invalid"; key: string; scope: "local" | "session"; message: string; recovered: "defaults" | "migrated" | "none" }
    | { type: "overlay.opened"; layer: number; ownerAttribute: string }
    | { type: "overlay.closed"; layer: number }
    | { type: "hmr.update"; file?: string; kind: "module" | "css" | "restart-required"; detail?: string }
    | { type: "devtools"; action: "requested" | "loaded" | "denied" | "failed"; reason?: string }
...
export type DiagnosticInput = DistributiveOmit<DiagnosticEvent, "id" | "at" | "level"> & { level?: DiagnosticLevel }
...
export function levelFor(type: DiagnosticEvent["type"]): DiagnosticLevel {
  if (type.endsWith(".failed") || type === "error" || type === "protocol.error" || type === "route.error" || type === "telemetry.failed") return "error"
  if (type === "shortcut.conflict" || type === "settings.invalid" || type === "storage.invalid" || type === "preflight.denied" || type === "manifest.retry") return "warn"
  if (type === "log" || type === "route.matched" || type === "overlay.opened" || type === "overlay.closed") return "debug"
  return "info"
}
