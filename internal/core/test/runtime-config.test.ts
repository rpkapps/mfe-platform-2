...
describe("runtime config", () => {
  it("generates from PLATFORM_* variables, refuses secrets, tracks sources", () => {
    const { config, refused, sources } = generateRuntimeConfig({
      env: {
        ...
        PLATFORM_MFE_ASSET_TRACKER_MANIFEST_URL:
          "https://cdn/asset-tracker/platform-manifest.json",
    ...
    expect(config.mfes["asset-tracker"]).toMatchObject({
      enabled: true,
      manifestUrl: "https://cdn/asset-tracker/platform-manifest.json",
      env: { API_BASE_URL: "https://api", PAGE_SIZE: 50 },
    })
    expect(config.mfes["legacy-reports"]).toMatchObject({ enabled: false })
    expect(refused).toEqual(
      expect.arrayContaining([
        "PLATFORM_MFE_ASSET_TRACKER_ENV_API_TOKEN",
        "PLATFORM_DB_PASSWORD",
      ])
    )
    expect(config.mfes["asset-tracker"]!.env.API_TOKEN).toBeUndefined()
    expect(sources["mfes.asset-tracker.manifestUrl"]).toBe(
      "PLATFORM_MFE_ASSET_TRACKER_MANIFEST_URL"
    )
  ...
  it("maps unknown ids by keyword boundary", () => {
    const { config } = generateRuntimeConfig({
      env: { PLATFORM_MFE_WIDGET_A_MANIFEST_URL: "u" },
    })
    expect(config.mfes["widget-a"]?.manifestUrl).toBe("u")
  })
  it("validates documents with actionable errors", () => {
    expect(() => parseRuntimeConfig({ mfes: { "Bad Id": {} } }, "test")).toThrowError(
      /Runtime configuration is invalid: mfes/
    )
  ...
  it("gives an MFE only its allow-listed view", () => {
    ...
    expect(
      redactRuntimeConfig(parseRuntimeConfig({ shared: { SOME_TOKEN: "t" } })).shared.SOME_TOKEN
    ).toBe("•••")
  })
})
