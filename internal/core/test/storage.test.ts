...
describe("storage store", () => {
  it("namespaces keys, validates, persists an envelope and notifies subscribers", () => {
    ...
    expect(JSON.parse(backend.get("local", store.namespacedKey)!)).toMatchObject({
      v: 1,
    ...
    expect(() => store.set({ columns: [], density: "loose" as never })).toThrowError(
      /fails its schema/
    )
    store.reset()
  ...
  it("recovers from malformed and invalid data with diagnostics", () => {
    ...
    expect(diagnostics).toHaveBeenCalledWith(
      expect.objectContaining({ code: "STORAGE_INVALID", recovered: "defaults" })
    )
  ...
  it("migrates older versions", () => {
    ...
    backend.set(
      "session",
      "platform:asset-tracker:session:dashboard",
      JSON.stringify({ v: 1, data: { cols: ["a"] } })
    )
    const store = createStorageStore({
      scope: "session",
      key: "dashboard",
      ...
      version: 2,
      migrate: (stored) => ({
        columns: (stored as { cols: string[] }).cols,
  ...
  it("receives cross-tab updates", () => {
    ...
    backend.emitExternal(
      "local",
      store.namespacedKey,
      JSON.stringify({ v: 1, data: { columns: ["remote"], density: "compact" } })
    )
  ...
  it("instance scoping changes the namespace", () => {
    ...
    const store = createStorageStore({
      scope: "local",
      key: "k",
      owner: { mfeId: "m", instanceId: "m#1", instanceScoped: true },
      defaults: 1,
      backend,
    })
