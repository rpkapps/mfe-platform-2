...
describe("entry URLs", () => {
  it("resolves baseUrl + file against the manifest URL and cache-busts with the buildId", () => {
    const url = computeEntryUrl(manifest(), {
      manifestUrl: "https://cdn.example.com/mfes/asset-tracker/platform-manifest.json",
    })
  ...
  it("respects absolute base URLs", () => {
    const url = computeEntryUrl(
      manifest({ remote: { baseUrl: "https://static.example.com/at/", preload: "none" } }),
      { manifestUrl: "https://app.example.com/m.json", cacheBust: false }
    )
  ...
  it("computes expose ids and preamble URLs", () => {
    ...
    ).toBe("http://localhost:5173/@platform/refresh-preamble")
  })
})
