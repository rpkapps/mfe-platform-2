...
import { applySharePolicy, hostShareScope, type ResolveShareArgs, type SharePolicyState } from "./share-policy"
...
async function defaultCreateInstance(options: FederationInstanceOptions): Promise<FederationInstance> {
 ...
 */
export function createModuleFederationLoader(options: ModuleFederationLoaderOptions = {}): ModuleFederationLoader {
  ...
  const sharePlugin = {
    ...
    resolveShare: (args: ResolveShareArgs) => {
      try {
        ...
        diagnostics?.emit({ type: "log", message: `share policy failed for ${args.pkgName}`, detail: String(error), level: "warn" })
        return args
      }
    },
  ...
  const register = async (manifest: MfeManifest, registerOptions: { manifestUrl: string; dev?: boolean; bustNow?: number }) => {
    const dev = registerOptions.dev ?? manifest.dev !== undefined
    const entryUrl = computeEntryUrl(manifest, { manifestUrl: registerOptions.manifestUrl, dev, cacheBust, bustNow: registerOptions.bustNow })
    const instance = await getInstance()
    instance.registerRemotes([remoteFor(manifest, entryUrl)], { force: true })
    const previous = registered.get(manifest.mfeId)
    registered.set(manifest.mfeId, { manifest, manifestUrl: registerOptions.manifestUrl, dev, entryUrl, preambleLoaded: previous?.preambleLoaded ?? false })
    nameToMfeId.set(manifest.entry.name, manifest.mfeId)
    diagnostics?.emit({ type: "remote.registered", loader: "module-federation", entryUrl, mfeId: manifest.mfeId })
    return entryUrl
  }
  ...
  const loadPreamble = async (manifest: MfeManifest, record: RegisteredRemote) => {
    ...
    try {
      ...
      diagnostics?.emit({ type: "hmr.update", kind: "module", detail: `refresh preamble installed from ${url}`, mfeId: manifest.mfeId, level: "debug" })
    } catch (error) {
      diagnostics?.emit({
        type: "log",
        level: "warn",
  ...
  const loader: ModuleFederationLoader = {
    ...
    get instance() {
      if (!instanceSync) throw new Error("The federation runtime is created on first use; await register() or load() first.")
      return instanceSync
    },
    ...
    async load(manifest, loadOptions) {
      ...
      if (!record || record.manifestUrl !== loadOptions.manifestUrl || record.manifest !== manifest) {
      ...
      for (let attempt = 1; attempt <= attempts + 1; attempt += 1) {
        if (loadOptions.signal?.aborted) {
          throw new PlatformError({ code: "REMOTE_LOAD_FAILED", message: `Loading ${mfeId} was aborted.`, owner: { mfeId }, source: record.entryUrl, details: { aborted: true } })
        }
        const startedAt = now()
        diagnostics?.emit({ type: "remote.loading", entryUrl: record.entryUrl, mfeId })
        try {
          const module = await instance.loadRemote<unknown>(id)
          const candidate = (module && typeof module === "object" && "default" in (module as object) ? (module as { default: unknown }).default : module) as unknown
          ...
          if (!isProtocolCompatible(PLATFORM_PROTOCOL_VERSION, candidate.protocolVersion)) {
            const error = new PlatformError({
              ...
              owner: { mfeId },
              source: record.entryUrl,
              details: { hostProtocol: PLATFORM_PROTOCOL_VERSION, remoteProtocol: candidate.protocolVersion },
            })
            diagnostics?.emit({ type: "protocol.error", code: "PROTOCOL_INCOMPATIBLE", error: error.toJSON(), mfeId })
            throw error
          }
          diagnostics?.emit({ type: "remote.loaded", durationMs: Math.max(0, now() - startedAt), mfeId })
          ...
          diagnostics?.emit({ type: "remote.failed", error: platformError.toJSON(), attempt, mfeId })
          const fatal = platformError.code === "PROTOCOL_INCOMPATIBLE" || platformError.details?.fatal === true
          if (fatal || attempt > attempts) break
          const delay = backoffMs * 2 ** (attempt - 1)
          diagnostics?.emit({ type: "manifest.retry", url: record.entryUrl, attempt, delayMs: delay, mfeId, level: "info" })
          if (delay > 0) await sleep(delay)
          if (cacheBust) {
            loader.invalidate(mfeId)
            await register(manifest, { manifestUrl: loadOptions.manifestUrl, dev: record.dev, bustNow: now() })
            record = registered.get(mfeId)!
          }
        }
      }
      throw lastError ?? new PlatformError({ code: "REMOTE_LOAD_FAILED", message: `Could not load ${mfeId}.`, owner: { mfeId } })
    },
    async preload(manifest, preloadOptions) {
      if (!registered.has(manifest.mfeId)) await register(manifest, { manifestUrl: preloadOptions.manifestUrl })
      const instance = await getInstance()
      if (!instance.preloadRemote) return
      try {
        await instance.preloadRemote([{ nameOrAlias: manifest.entry.name, exposes: [manifest.entry.expose.replace(/^\.\//, "./")], resourceCategory: "sync" }])
      } catch (error) {
        diagnostics?.emit({ type: "log", level: "warn", message: `Preloading ${manifest.mfeId} failed`, detail: error instanceof Error ? error.message : String(error), mfeId: manifest.mfeId })
      }
    },
    sharedReport(mfeId) {
      ...
      return buildSharedReport({ manifest: record.manifest, federationName, recorded: policy.resolutions.get(federationName), shareScopeMap })
    },
    invalidate(mfeId) {
      const record = registered.get(mfeId)
      if (!record) return
      ...
      try {
        ...
        if (cache) {
          for (const key of Array.from(cache.keys())) if (key === federationName || key === mfeId) cache.delete(key)
        }
      } catch {
