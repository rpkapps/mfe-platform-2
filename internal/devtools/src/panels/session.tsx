    ...
    <Section title="Session and user (redacted)">
      <KeyValue
        entries={[
          [
            "User",
            session.user ? `${session.user.displayName} (${session.user.id})` : "anonymous",
          ],
          [
            "Permission groups",
            `${session.groupsCount} group${session.groupsCount === 1 ? "" : "s"} (names not shown)`,
          ],
          ...
          [
            "Shell release",
            [session.release.version, session.release.buildId, session.release.commit]
              .filter(Boolean)
              .join(" · ") || "—",
          ],
