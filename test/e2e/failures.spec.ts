...
test.describe("failure isolation", () => {
  test("every boundary fails alone with an actionable diagnostic", async ({ page }) => {
    ...
    await expect(page.locator('[data-failure-case="broken-remote"]')).toContainText("MANIFEST_INVALID", { timeout: 30_000 })
    await expect(page.locator('[data-failure-case="incompatible-remote"]')).toContainText("PROTOCOL_INCOMPATIBLE")
    await expect(page.locator('[data-failure-case="unavailable-remote"]')).toContainText("MANIFEST_FETCH_FAILED", { timeout: 40_000 })
    await expect(page.locator('[data-failure-case="disabled-remote"]')).toContainText("REMOTE_DISABLED")
    await expect(page.locator('[data-failure-case="restricted-remote"]')).toContainText("PERMISSION_DENIED")
    // Docs links and retry are offered.
    await expect(page.locator('[data-failure-case="unavailable-remote"] a[href*="docs"]')).toHaveCount(1)
    await expect(page.locator('[data-failure-case="unavailable-remote"]').getByRole("button", { name: /Retry/ })).toBeVisible()
  ...
  test("manifest URL overrides at runtime with visible source", async ({ page }) => {
    await gotoShell(page, "/asset-tracker?platform.override.asset-tracker=http://127.0.0.1:4999/platform-manifest.json")
    await expect(page.getByTestId(ids.shell.outlet)).toContainText("MANIFEST_FETCH_FAILED", { timeout: 40_000 })
