...
test.describe("settings and storage", () => {
  test("framework-managed groups render, persist, validate and reset", async ({ page }) => {
    ...
    await expect(showOffline).toBeVisible()
    await showOffline.click()
    await expect(host.getByText("Favourite sites")).toBeHidden()
    const pageSize = host.getByLabel(/Page size/)
    await pageSize.fill("3")
    await pageSize.blur()
    await expect(host.getByText(/at least 5|>=5|Too small|minimum/i)).toBeVisible()
    await pageSize.fill("50")
    await pageSize.blur()
    await expect(host.getByRole("combobox", { name: /Default region/ })).toBeVisible()
    await page.reload()
    await expect(page.getByTestId(ids.shell.settingsHost).getByText("Display")).toBeVisible({ timeout: 30_000 })
    ...
    const stored = await page.evaluate(() => Object.keys(localStorage).filter((key) => key.startsWith("platform:asset-tracker:settings")))
  ...
  test("invalid stored settings recover to defaults without crashing", async ({ page }) => {
    ...
    await page.evaluate(() => {
      localStorage.setItem("platform:asset-tracker:settings:asset-tracker:display.pageSize", JSON.stringify({ v: undefined, value: "garbage" }))
    })
    await page.goto("/settings")
    const host = page.getByTestId(ids.shell.settingsHost)
    await expect(host.getByText("Display")).toBeVisible({ timeout: 30_000 })
    await host.getByText("Display").click()
    await expect(host.getByLabel(/Page size/)).toHaveValue("25")
    await expect(host.getByText(/reset to its default|recovered|invalid/i)).toBeVisible()
  })

  test("schema-backed storage is namespaced, persists and syncs across tabs", async ({ page, context }) => {
    await gotoShell(page, "/asset-tracker")
    await waitForAssetTracker(page)
    await page.getByTestId(ids.assetTracker.storageAdd).click()
    await expect(page.getByTestId(ids.assetTracker.storageColumns)).toHaveText("name,status,col3")
    const keys = await page.evaluate(() => Object.keys(localStorage))
    expect(keys).toContain("platform:asset-tracker:local:dashboard")
    const second = await context.newPage()
    await second.goto("/asset-tracker")
    await expect(second.getByTestId(ids.assetTracker.storageColumns)).toHaveText("name,status,col3", { timeout: 30_000 })
    await second.getByTestId(ids.assetTracker.storageAdd).click()
    await expect(page.getByTestId(ids.assetTracker.storageColumns)).toHaveText("name,status,col3,col4")
    ...
    await page.evaluate(() => localStorage.setItem("platform:asset-tracker:local:dashboard", "{broken"))
    await page.reload()
    await waitForAssetTracker(page)
    await expect(page.getByTestId(ids.assetTracker.storageColumns)).toHaveText("name,status")
  })
  ...
  test("MFE-managed settings page owns its own state", async ({ page }) => {
    ...
    await expect(page.getByTestId(ids.assetTracker.customSettings)).toBeVisible({ timeout: 30_000 })
    await page.getByLabel(/Session notes/).fill("hello")
    await expect(page.getByTestId(ids.assetTracker.customSettingsValue)).toHaveText("5 characters")
