...
test.beforeAll(async () => {
  const env = {
    ...
    ...conformanceEnv({
      mode: "development",
      ports: { assetTracker: 4301, legacyReports: 4302, widgetA: 4303, widgetB: 4304 },
    }),
  }
...
test("HMR updates React 19 and React 18 remotes while shell and remote state survive", async ({
  ...
  edit(
    join(root, "apps/conformance-react19/src/routes/index.tsx"),
    "HMR_LABEL_V1",
    "HMR_LABEL_V2"
  )
  await expect(page.getByTestId(ids.assetTracker.hmrLabel)).toHaveText("HMR_LABEL_V2", {
    timeout: 30_000,
  })
  ...
  await page
    ...
    .nth(1)
    .getByTestId(ids.widgets.counterWidgetIncrement)
    .click()
  edit(
    join(root, "apps/conformance-react18/src/widgets/report-summary.tsx"),
    "WIDGET_HMR_V1",
    "WIDGET_HMR_V2"
  )
  ...
  await expect(
    page
      .getByTestId(ids.widgets.counterWidget)
      .nth(1)
      .getByTestId(ids.widgets.counterWidgetValue)
  ).toHaveText("2")
...
test("restart-requiring changes produce a clear diagnostic instead of a stale remote", async ({
  ...
  edit(
    ...
    'displayName: "Asset Tracker (renamed)"'
  )
  await page.goto("/asset-tracker")
