...
export function AssetCard({ assetId, compact }: { assetId: string; compact?: boolean }) {
  ...
  useRegisterCommand({ id: "open-asset", label: `Open ${asset?.name ?? assetId}`, group: "Widgets", handler: () => navigation.navigate(`/asset-tracker/assets/${assetId}`) })
  return (
    <Card data-testid={ids.assetCard} data-asset={assetId} className={compact ? "py-2" : undefined}>
      <CardHeader>
      ...
      <CardContent className="flex gap-2">
        ...
        <Button size="sm" variant="outline" onPress={() => navigation.navigate(`/asset-tracker/assets/${assetId}`)}>
          Open
        </Button>
      </CardContent>
    </Card>
