...
export function renderLlmsTxt() {
  ...
  lines.push("")
  lines.push(
    "Four public packages: @platform/react (MFE SDK), @platform/vite (Vite plugin), @platform/cli (scaffolding, dev, lint) and @platform/host (shell runtime). Canonical project instructions for agents: " +
...
- @platform/react  MFE SDK (React 18 or 19): createMfe, createWidget, hooks, storage, registrations, testing
...
pnpm dev        # platform dev: Vite + local shell harness at /__platform/harness/ (HMR, context, failure toggles)
...
src/mfe.tsx                 bootstrap: export default createMfe({ routeTree, widgets })
src/routes/__root.tsx       createRootRouteWithContext<MfeRouterContext>()
src/routes/**               TanStack file routes (relative to the route prefix)
src/routeTree.gen.ts        generated
src/widgets/*.tsx           createWidget({ id, component, propsSchema })
src/commands.tsx            useRegisterCommand
src/settings.ts             useRegisterSettingsGroup
src/storage.ts              createPlatformStorage
src/help.ts, src/release-notes.ts
src/env.d.ts                declare module "@platform/react" { interface Register { env; featureFlags } }
...
Route with guard, loader, breadcrumb (src/routes/assets/$assetId.tsx):
  export const Route = createFileRoute("/assets/$assetId")({
    ...
    loader: ({ context, params }) => { const span = context.platform.telemetry.span("load-asset"); return fetchAsset(params.assetId).finally(() => span.end()) },
    staticData: { breadcrumb: { fromLoader: "name" }, navigation: { title: "Asset" }, permissionGroups: ["assets:read"] },
    component: AssetPage,
  })
...
Storage:
  ...
  store.use((s) => s.columns); store.set(...); store.setKey("columns", ...); store.reset()

Widget:
  export const assetCard = createWidget({ id: "asset-card", propsSchema: z.object({ assetId: z.string() }), component: AssetCard })
...
Overlays: ordinary Tecton Dialog/Popover/Menu; no configuration (PortalProvider adapter + shell overlay manager)
...
createPlatformHost({ runtimeConfig, registry, loader?, telemetry?, storage?, navigation?, context?, policy? })
@platform/host/react: PlatformProvider, MfeOutlet, WidgetSlot, CommandPalette, SettingsHost, HelpSlot, ReleaseNotesSlot, Breadcrumbs, AppFinder, PlatformDevtools, NotificationHost
@platform/host/tanstack: createTanStackShellNavigation(router), mfeRoute({ mfeId })
...
Runtime config: platform-host-entrypoint --out platform-config.json from PLATFORM_* variables
...
New SDK hook: packages/react/src + export + CAPABILITY_BY_API in core if it implies a capability + docs page + lint rule if misuse is statically detectable
