  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <p className="text-muted-foreground text-sm">
        Every widget below runs in its own React root: React 19 (asset-tracker, widget-a) and
        React 18 (legacy-reports, widget-b) coexist.
      </p>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <WidgetSlot
          ...
          widgetId="kpi-tile"
          slot="kpi-1"
          props={{ label: "Production", value: kpi, unit: "bbl/d" }}
        />
        <WidgetSlot
          mfeId={MFE_IDS.widgetA}
          widgetId="kpi-tile"
          slot="kpi-2"
          props={{ label: "Uptime", value: 99, unit: "%" }}
        />
        <WidgetSlot
          mfeId={MFE_IDS.widgetA}
          widgetId="modal-widget"
          slot="modal-a"
          props={{ label: "Open React 19 dialog" }}
        />
        <WidgetSlot
          mfeId={MFE_IDS.legacyReports}
          widgetId="report-summary"
          slot="report-summary"
          props={{ reportId: "daily-production" }}
        />
        <WidgetSlot
          mfeId={MFE_IDS.legacyReports}
          widgetId="stacked-modal"
          slot="modal-18"
          props={{ label: "Open React 18 modal" }}
        />
        {Array.from({ length: counters }, (_, index) => (
          <WidgetSlot
            ...
            slot={`counter-${index}`}
            props={{ label: `Counter ${index + 1}`, step: index + 1 }}
          />
        ))}
        <WidgetSlot
          mfeId={MFE_IDS.widgetA}
