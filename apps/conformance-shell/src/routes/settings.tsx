    ...
    return (
      <div data-testid={TEST_IDS.shell.settingsHost} className="flex flex-col gap-4">
        ...
        <p className="text-muted-foreground text-sm">
          Settings registrations are lifecycle-bound, so this shell mounts the settings owners
          headlessly while the page is open. Framework-managed groups render here; MFE-managed
          groups link to their own page.
        </p>
        ...
        <div hidden aria-hidden="true" data-settings-owners>
          {host ? (
            <MfeOutlet mfeId={MFE_IDS.assetTracker} routePrefix="/asset-tracker" />
          ) : null}
          {host ? (
            <MfeOutlet mfeId={MFE_IDS.legacyReports} routePrefix="/legacy/reports" />
          ) : null}
        </div>
      </div>
    )
