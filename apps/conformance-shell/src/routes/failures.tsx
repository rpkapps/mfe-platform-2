...
const cases = [
  {
    ...
    detail: "The manifest fails validation (MANIFEST_INVALID).",
  },
  {
    mfeId: MFE_IDS.incompatible,
    title: "Incompatible protocol",
    detail: "Protocol 99 is not loadable (PROTOCOL_INCOMPATIBLE).",
  },
  {
    mfeId: MFE_IDS.unavailable,
    title: "Unavailable remote",
    detail:
      "Nothing listens on the manifest URL; retries with cache busting (MANIFEST_FETCH_FAILED).",
  },
  {
    mfeId: MFE_IDS.disabled,
    title: "Disabled by runtime configuration",
    detail: "`mfes.disabled-remote.enabled = false` (REMOTE_DISABLED).",
  },
  {
    mfeId: MFE_IDS.restricted,
    title: "Permission preflight",
    detail: "Requires the `superadmin` group nobody has (PERMISSION_DENIED).",
  },
]
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <p className="text-muted-foreground text-sm">
        Each boundary below fails in isolation: the shell and the other remotes keep working.
      </p>
      <div className="grid gap-4 md:grid-cols-2">
        {cases.map((item) => (
          <section
            key={item.mfeId}
            data-failure-case={item.mfeId}
            className="border-border rounded-md border p-3"
          >
            <h2 className="font-medium">{item.title}</h2>
            <p className="text-muted-foreground mb-2 text-xs">{item.detail}</p>
