...
export function AppFinder({ shellName = "Home", className, extra = [] }: AppFinderProps) {
      ...
      h.remotes
        ...
        .filter(
          (record) =>
            record.enabled &&
            record.discoverable &&
            (!record.manifest || record.manifest.kind === "mfe")
        ),
  ...
  const groups = useMemo(() => {
    ...
    for (const record of remotes) {
      ...
      map.set(category, [
        ...(map.get(category) ?? []),
        {
          id: record.mfeId,
          name,
          href: host.routePrefixOf(record.mfeId),
          description: manifest?.navigation?.description ?? manifest?.description,
          keywords: [record.mfeId, ...(manifest?.navigation?.keywords ?? [])],
          order: manifest?.navigation?.order ?? 100,
        },
      ])
    }
    for (const app of extra)
      map.set(app.category ?? "Shell", [
        ...(map.get(app.category ?? "Shell") ?? []),
        {
          id: app.id,
          name: app.name,
          href: app.href,
    ...
    return Array.from(map.entries()).map(([category, apps]) => ({
      category,
      apps: apps.sort((a, b) => a.order - b.order || a.name.localeCompare(b.name)),
    }))
  }, [remotes, extra, host])
  const currentApp = groups
    .flatMap((group) => group.apps)
    .find((app) => app.id === current?.mfeId)
            ...
            <AppFinderGroup key={group.category} heading={group.category}>
              {group.apps.map((app) => (
                <AppFinderItem
                  ...
                  description={app.description}
                  keywords={app.keywords}
                  icon={codeOf(app.name)}
                  tone={toneOf(app.id)}
                  isCurrent={app.id === current?.mfeId}
                />
              ))}
            </AppFinderGroup>
          ))}
